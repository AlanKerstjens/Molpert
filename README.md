...
To be able to import the library from Python add `${MOLPERT}/lib` to your `${PYTHONPATH}`. Consider doing so in your `bashrc` file. Otherwise you'll have to manually extend `${PYTHONPATH}` everytime you open a new shell.
...
Virtually everything you need can be found in the `MoleculePerturber.hpp` header. Make sure  `Molpert/source` is in your compiler's include path.
...
Perturbations are specifications of how a molecule ought to be modified. They are represented as classes that inherit from the `MolecularPerturbation` base class. There are 8 perturbation types of interest to the average user. Each class has a corresponding entry in the `MolecularPerturbation::Type` enum.
...
After construction a perturbation can be executed by calling its `operator()`. Said operator has two overloads: one to perturb mutable `RDKit::RWMol`s in place, and one to perturb copies of immutable `RDKit::ROMol`s.

```cpp
RDKit::RWMol rwmol (...);
RDKit::ROMol romol (...);
TagMolecule(rwmol); // Explained in next section
TagMolecule(romol); // Explained in next section
...
## Tagging molecules
In the RDKit you can reference a specific atom or bond of a molecule through their pointers (`RDKit::Atom*` and `RDKit::Bond*`) or indices. Since atoms and bonds are stored in contiguous memory both their addresses and indices may be invalidated when you modify the topology of a molecule. This makes keeping track of atoms and bonds challenging.

To keep track of an atom or bond throughout a molecule's lifespan we use tags. Tags are integers attached to atoms and bonds. They remain constant regardless of how we modify the molecule and aren't re-used within the same molecule.
...
TagMolecule(molecule);   // Tag the molecule
```
...
You can also retrieve an atom or bond with a specific tag. This relies on a plain for loop, so use it sparingly.

```cpp
const RDKit::Atom* atom = GetAtomWithTag(molecule, 0);
const RDKit::Bond* bond = GetBondWithTag(molecule, 0);
```

## Manually constructing perturbations

You can construct perturbations by manually calling their constructor.

```cpp
AtomicNumberChange perturbation (
...
The first example is rather silly, as it's more convoluted than calling the corresponding RDKit setter (`RDKit::Atom::setAtomicNum()`). The second example is more interesting as it deletes the specified atom and creates some new bonds to ensure the molecule remains connected. Note that we don't need to specify which bonds ought to be formed.
...
*Sampling values* are vectors from which atom and bond properties are sampled. Each value array has a corresponding size-matched vector of *sampling weights*. The default values and weights are derived from ChEMBL. If you manually modify the value vector make sure you modify the weights vector accordingly. In the following example you would set a 60% probability of sampling carbons and a 20% probability of sampling nitrogens and oxygens. Atomic numbers in particular are sampled during `AtomicNumberChange`s and `AtomInsertions`.
...
You can quickly modify some of these arrays by passing arguments to the `MoleculePerturber` constructor. This is less error prone than doing it manually.
...
You can also adjust which perturbation types are sampled and with which probabilities. The main difference with the above is that perturbation sampling settings are fixed-size variables, and must be modified with `operator[]` instead of `operator=`. By default all perturbations may be sampled, with sampling weights being derived from ChEMBL.

```cpp
perturber.perturbation_types[MolecularPerturbation::Type::AtomInsertion_t] = false;
perturber.perturbation_types_weights[MolecularPerturbation::Type::BondDeletion_t] *= 2.0;
```

### Stochastic perturbation construction

`MoleculePerturber` has member functions to randomly generate eligible perturbations. These functions rely on a random number generator which must be passed as argument to the perturbation generators.
...
If you don't mind the type of the generated perturbations you can call `MoleculePerturber::operator()`:

```cpp
RDKit::ROMol molecule (...);
TagMolecule(molecule);
// Perturb molecule in any way
std::shared_ptr<MolecularPerturbation> perturbation = perturber(
  molecule, prng);
```

To limit the generation to a specific type of perturbation call the corresponding member function. Stochastic perturbation generators are named using the perturbation type in imperative tense:
...
If you'd like to enumerate applicable perturbations of all types call `MoleculePerturber::operator()`:
...
bool NoHeteroBond(const RDKit::Bond* bond) {
  return !(IsHeteroatom(bond->getBeginAtom()) && IsHeteroatom(bond->getEndAtom()));
};

bool LessThan50HeavyAtoms(const RDKit::ROMol* molecule) {
  return molecule->getNumHeavyAtoms() <= 50;
};
...
constraints.SetBondConstraint(2, NoHeteroBond);
// Molecule may not have more than 4 rotatable bonds.
constraints.SetMoleculeConstraint(IsRigid(4));
```

Pass the `MolecularConstraints` to the perturbation generator as the last argument:
...
```cpp
constraints.SetMoleculeConstraint(LessThan50HeavyAtoms); // OK
constraints.SetMoleculeConstraint(IsRigid(4)); // OK
...
If you'd like to remove an upper limit for a setting set it to `std::numeric_limits<unsigned>::max()`. The code will check against this value and skip unnecessary calculations. DO NOT set it to an arbitrarily high value, as the graph traversals will still be executed.
